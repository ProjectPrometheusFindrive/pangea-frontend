...
export default function Reservations() {
  ...
  const legacyReservationPaymentStatus = selectedReservation
    ...
    : null;
  const canEditLegacyReservationPaymentMethod = legacyReservationPaymentStatus === 'partial'
    ? canConfirmPayment
    : canCreateCharge;
  const canEditReservationPaymentMethod = selectedReservation
    ? canEditLegacyReservationPaymentMethod
      && !hasSelectedReservationBillingLedger
      && canManageReservationPaymentIssue(selectedReservation, selectedReservationPaymentSync)
    : false;
  const canEditReservationAdditionalAmount = selectedReservation
    ? canCreateCharge
      && !hasSelectedReservationBillingLedger
      && canManageReservationPaymentIssue(selectedReservation, selectedReservationPaymentSync)
    : false;
  const selectedReservationId = selectedReservation?.id ?? null;
  const resolvedAdditionalPaymentAmount = selectedReservation
    ? resolveReservationAdditionalPaymentAmount(selectedReservation, selectedReservationPaymentSync)
    : 0;
  const resolvedReservationPaymentMethod = selectedReservation
  ...
  useEffect(() => {
    ...
    setPaymentAmountDraft(String(Math.max(0, Math.trunc(resolvedAdditionalPaymentAmount))));
  }, [selectedReservationId, resolvedAdditionalPaymentAmount]);
  useEffect(() => {
    if (!selectedReservationId) {
      setPaymentMethodDraft('카드');
      return;
    }
    setPaymentMethodDraft(resolvedReservationPaymentMethod);
  }, [selectedReservationId, resolvedReservationPaymentMethod]);
  useEffect(() => {
  ...
  const handleUpdateReservationPaymentStatus = useCallback(async (
    ...
    const statusPermission = nextStatus === 'canceled' ? canVoidPayment : canConfirmPayment;
    if (!statusPermission) {
  ...
  const handleSaveReservationPaymentMethod = useCallback(async () => {
    const baseStatus = selectedReservationPaymentSync?.status === 'not-found'
      ? toCanonicalPaymentStatus(selectedReservation?.paymentStatus)
      : selectedReservationPaymentSync?.status ?? toCanonicalPaymentStatus(selectedReservation?.paymentStatus ?? 'pending');
    const statusPermission = baseStatus === 'partial' ? canConfirmPayment : canCreateCharge;
    if (!statusPermission) {
  ...
  const handleCreateManualChargeItem = useCallback(async () => {
    ...
    if (chargeItemDraft.status === 'waived' && !canWaivePayment) {
      setReservationActionError('청구 항목 면제 권한이 없습니다.');
      return;
    }
                ...
                {activeTab === 'payment' && (
                  <div className="space-y-3">
                    ...
                    <div className="rounded-lg border border-gray-200 bg-white p-4">
                      ...
                      {(canCreateCharge || canCreatePayment) && (
                                ...
                                <label className="text-xs font-semibold text-gray-600">
                                  ...
                                  >
                                    {MANUAL_CHARGE_STATUS_OPTIONS
                                      ...
                                      .map((option) => (
                                      <option key={option.value} value={option.value} disabled={option.value === 'waived' && !canWaivePayment}>{option.label}</option>
                                    ))}
                                  </select>
                                </label>
                                <label className="text-xs font-semibold text-gray-600">
                            ...
                            {selectedReservationMonthlyChargeItems.length === 0 ? (
                              ...
                              >
                                ...
                                <span className="flex flex-wrap gap-1">
                                  ...
                                  {(canWaivePayment || canCreateCharge) && !['paid', 'waived', 'refunded', 'disputed'].includes(item.status) && (
                                    <>
                                  ...
                                  {canCreateCharge && item.status === 'disputed' && (
                                    <button
                                      type="button"
                            ...
                            {selectedReservationOtherChargeItems.map((item) => (
                              ...
                              >
                                ...
                                <span className="min-w-0 space-y-1">
                                  ...
                                  {canCreatePayment && canConfirmPayment && canAllocatePayment && item.chargeType !== 'refund' && item.remainingAmount > 0 && !['paid', 'waived', 'refunded', 'disputed'].includes(item.status) && (
	                                    <button
	                                      type="button"
	                                      onClick={() => {
	                                        openSettleChargeItemConfirmation(item);
                                  ...
                                  {(canWaivePayment || canCreateCharge) && item.chargeType !== 'refund' && !['paid', 'waived', 'refunded', 'disputed'].includes(item.status) && (
                                    <span className="flex min-w-0 flex-wrap gap-1">
                                      {canWaivePayment && <button
                                        type="button"
                                        onClick={() => {
                    ...
                    {!hasSelectedReservationBillingLedger && (
                      ...
                      <div className="flex items-center justify-between gap-3">
                        ...
                        {canEditReservationPaymentMethod ? (
                          <div className="flex items-center gap-2">
                            <select
                      ...
                      <div className="flex items-center justify-between gap-3">
                        ...
                        {canEditReservationAdditionalAmount ? (
                          <div className="flex items-center gap-2">
                            <input
                              type="text"
