...
test('Assets restores a model filter with catalog-backed options, counts, and pagination', () => {
  ...
  assert.match(source, /const loadAssetsCatalog = useCallback\(async \(\s*signal: AbortSignal,\s*options: \{ status\?: string; query\?: string \} = \{\},\s*\) => \{/u);
  ...
  assert.match(source, /const pageAssets = paginateAssets\(filteredCatalog, page, pageSize\);/u);
  assert.match(source, /return \{[\s\S]*items:\s*pageAssets,[\s\S]*total:\s*filteredCatalog\.length,/u);
  ...
  assert.match(source, /const statusCountMap = useMemo\(\(\) => \(\{\s*rental: catalogAssets\.filter/u);
