...
test('new contract modal splits long-term contractor, driver, and payer validation', () => {
  ...
  assert.match(source, /const \[contractorType, setContractorType\] = useState<'individual' \| 'corporate'>\('corporate'\);/u);
  ...
  assert.match(source, /data-testid="new-contract-payer-type-input"/u);
});

test('new contract modal treats accident replacement driver fields as pre-pickup data', () => {
  const source = readProjectFile('src/app/components/NewContractModal.tsx');

  assert.match(source, /const requiresDriverAtCreation = rentalType !== 'accident_replacement';/u);
